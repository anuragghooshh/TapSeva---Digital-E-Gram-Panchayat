...
const Input: React.FC<InputProps> = ({ type, placeholder, id, name, required=false, value, onChange }) => {
  return (
    <input
      ...
      value={value}
      onChange={onChange}
      className="block rounded-sm w-full font-work text-dark py-3 px-4 bg-gray-100 border focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
    />
  )
}
...
const TextArea: React.FC<TextAreaProps> = ({ placeholder, id, name, required, value, onChange }) => {
  return (
    <textarea
      ...
      value={value}
      onChange={onChange}
      className="block w-full font-work text-dark py-3 px-4 bg-gray-100 border focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
    />
  )
}
...
const Option: React.FC<OptionProps> = ({ children, value, disabled }) => {
  return (
    <option
      ...
      className="text-dark font-work py-2 px-3 bg-light"
    >
      {children}
    </option>
  )
...
const Select: React.FC<SelectProps> & { Option: React.FC<OptionProps> } = ({ children, name, id, value, onChange }) => {
  return (
    <select
      ...
      value={value}
      onChange={onChange}
      className="w-full rounded-sm font-work text-dark py-3 px-4 bg-gray-100 border border-tertiary focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
    >
      {children}
    </select>
  )
